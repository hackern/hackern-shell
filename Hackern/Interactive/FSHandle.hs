...
handleLs shellState@(ShellState_ here xs con _)= do
  handle <- openDir here
  dirInfo <- readDir handle
  lift $ writeConsole con $ printDir dirInfo ++ "\n"
  return shellState

handleCd to shellState@(ShellState_ here xs con dev) = case to of
  ".." -> return $ ShellState_ (takeDirectory here) xs con dev
  d    -> do
    handle <- openDir here
    dirInfo <- readDir handle
    if (filter (== d) $ map fst dirInfo) /= [] then
      return $ ShellState_ (here </> d) xs con dev
      else do
        lift $ writeConsole con "No such directory\n"
        return shellState

handleMkdir path shellState@(ShellState_ here xs con _) = do
  if path /= ".."
